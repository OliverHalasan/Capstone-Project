...
$result = mysqli_query($conn, "SELECT * FROM candidate WHERE user_id = $user_id") or die("Error: " . mysqli_error($conn));

$current_user = mysqli_fetch_array($result);
mysqli_query($conn, "UPDATE candidate SET instagram_key = '$token' WHERE user_id = $user_id") or die("Error: " . mysqli_error($conn));
